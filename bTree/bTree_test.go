...
func TestInsertOneIndex(t *testing.T) {
	...
	if err != nil || !slices.Contains((*tree.indexes)[:], expected) {
		t.Errorf(`pushIndex(5) should return %v instead it returns: %v`, expected, err)
	}
}
...
//
// 	if err != nil || !slices.Equal(indexes[:], expected) {
